/**
 * TODO: legacy definition from old logdown about.md, refactor with readable structure and drop html usage
 */
type Friend = [nick: string, githubId: string, link: string, descriptionHtml: string]

const friends: Friend[] = [
    ['cold', 'coldnight', 'http://www.linuxzen.com/', '最开始折腾博客给予我很多支持的 cold 大大。'],
    ['晓风', 'dongdxf', 'http://www.dongxf.com/', '最开始折腾博客认识的 Pelican 同伴。'],
    ['Lazy Mind', 'mapleray', 'http://lazymind.me', '低调沉稳的学长。'],
    ['星光', 'starlightme', 'http://jimmy66.com/', '在现代魔法研究协会相遇的闪闪的星光。'],
    ['Ayase-252', 'Ayase-252', 'http://blog.ayase.moe/', '可爱的自动化<del>LL神教徒</del>学长。'],
    ['飞狐', 'lucumt', 'http://lucumt.info/', '活动于帝都的程序猿前辈。'],
    [
        'FarseerFc',
        'farseerfc',
        'http://farseerfc.me/',
        '<del>远见 RFC，爱复习</del>；经验丰富的 Arch Linux TU，人生赢家 fc 前辈。',
    ],
    ['EAGzzyCSL', 'EAGzzyCSL', 'http://eagzzycsl.github.io/', '同门室友，安静敲代码的男子。'],
    [
        'Lyken Syu',
        'Lyken17',
        'http://lykensyu.com/',
        '想要追上我们仍未知道那个夏天要以多快的速度前进才能企及的魔法少年力霸是否搞错了什么。',
    ],
    [
        'Phoenix Nemo',
        'phoenixlzx',
        'https://blog.phoenixlzx.com/',
        '好吃的凤凰卷，绘触（看头像），而且还是个好人 (′ʘ⌄ʘ‵)',
    ],
    [
        'Felix Yan',
        'felixonmars',
        'http://felixc.at/',
        '火星猫，肥猫；带领众 Archer <del>一起吞并全太阳系发行版</del>的肥猫领袖。',
    ],
    ['acgtyrant', 'acgtyrant', 'http://acgtyrant.com', '御宅暴君；一本正经的萌。'],
    [
        'quininer kel',
        'quininer',
        'https://quininer.github.io/',
        '奎尼；<del>你怎么还没有用 Tox 啊？</del>好棒！好棒！好棒！',
    ],
    ['CUI Hao', 'cuihaoleo', 'http://i-yu.me/', '催土豪；有爱的 Arch Linux 中文社区源的维护者。'],
    [
        '谷月轩',
        'LastAvenger',
        'http://lastavenger.github.io/',
        'LastAvengers，LA；又会画画又会写内核又会弹琴的 LA 大大。',
    ],
    ['Steve Zhang', 'Stevearzh', 'http://stevearzh.me/', '前端大前辈史蒂夫。'],
    ['ヨイツの賢狼ホロ', 'KenOokamiHoro', 'https://blog.yoitsu.moe/', '抱一下萌狼尾巴。'],
    ['依云', 'lilydjwg', 'http://lilydjwg.is-programmer.com/', '百合仙子；掌握各种技术经验丰富的仙子前辈。'],
    ['謝宇恆', 'xieyuheng', 'http://xieyuheng.github.io/', 'xyh，小妖狐，小药壶；语言设计之小药壶。'],
    ['夏娜(VOID001)', 'VOID001', 'https://void-shana.moe/', '向着内核大嗨卡前进的夏娜君。'],
]

export default friends
