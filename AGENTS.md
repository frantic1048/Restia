...
- `RESTIA_E2E_URL=http://127.0.0.1:<port> yarn playwright test tests/post-entry-layout.spec.ts --project=chromium --reporter=line` - Focused regression test for homepage post-entry layout stability while cover images load

Node version: 24.14.0 (see `.node-version`, managed by proto)

When the system `node`/`yarn` resolves to a broken Homebrew Node, prefer the proto shims: `PATH=/Users/kafuuchino/.proto/shims:$PATH yarn ...`.

Astro may choose another port if `4321` is occupied; pass the actual URL through `RESTIA_E2E_URL` when running Playwright.
...
- **Responsive image artifacts**: `src/util/images.ts` stores reusable processed images in `.cache/restia-images/` and materializes served files into `public/_img/` for dev plus `dist/_img/` during production builds.
